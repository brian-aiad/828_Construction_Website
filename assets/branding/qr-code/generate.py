...
qr = qrcode.QRCode(version=8, error_correction=qrcode.constants.ERROR_CORRECT_H,
                   box_size=SCALE, border=BORDER, mask_pattern=4)
...
# Use the supplied logo as a stencil: black lettering and white negative space.
# Ignore near-transparent background residue, retaining all opaque letter shapes.
alpha = np.array(source_logo)[:, :, 3]
stencil = Image.fromarray(np.where(alpha > 128, 0, 255).astype(np.uint8))
ys, xs = np.where(alpha > 128)
crop = (int(xs.min()), int(ys.min()), int(xs.max())+1, int(ys.max())+1)
logo = stencil.crop(crop).convert("RGB")

# Module-aligned clearance keeps the center badge away from functional patterns.
# Match the original 21-module square clearance and thin frame.
bx, by, bw, bh = (units - 21) / 2, (units - 21) / 2, 21, 21
inset = 0.46
...
# Scalable QR modules, with the supplied logo as black lettering on white.
...
report = {"url": URL, "type": "static; direct URL; no QR provider or subscription",
          ...
          "center_clearance_modules": [bw,bh], "source_logo_sha256": hashlib.sha256(logo_path.read_bytes()).hexdigest(),
          "mask_pattern": 4, "logo_presentation": "Original letter shapes as black on white; original reference spacing", "logo_crop": crop,
